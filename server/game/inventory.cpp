...
bool Inventory::equip(Item& item,
                      Player& player) {  // equipar puede ser que se agregue al equipment o que se
                                         // consuma dependiendo del tipo de item
    ...
    Item* prev =
        equipment.equip(items[index].get(), *slot);  // tomo el item anterior del slot si existe
...
bool Inventory::unequip(EquipmentSlot slot) {
    ...
    if (!is_full()) {
        items.push_back(
            std::unique_ptr<Item>(item));  // si el inventario no está lleno vuelve al inventario
    } else {
        delete item;  // drop
    }
    return true;
