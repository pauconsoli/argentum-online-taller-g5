...
int GameFormulas::calculate_max_hp(const Player& player) {
    ...
    float class_multiplier =
            config.get_class_hp_multiplier(player.get_class());

    float race_multiplier =
            config.get_race_hp_multiplier(player.get_race());

    // Tiene más sentido que el resultado sea un int, se puede cambiar
...
int GameFormulas::calculate_max_mana(const Player& player) {
    ...
    float class_multiplier =
            config.get_class_mana_multiplier(player.get_class());

    float race_multiplier =
            config.get_race_mana_multiplier(player.get_race());

    // Tiene más sentido que el resultado sea un int, se puede cambiar
    return static_cast<int>(
            player.get_intelligence() *
            class_multiplier *
            race_multiplier *
            player.get_level());
}
...
int GameFormulas::calculate_max_gold(const Player& player) {
    ...
    // Tiene más sentido que el resultado sea un int, se puede cambiar
    return static_cast<int>(
            100 * std::pow(player.get_level(), 1.1));
}